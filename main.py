...
class portfolio:
    ...
    def __init__(self, stocks, start_date, end_date, rf, weight_start, weight_end, n):
        ...
        for i in range(n):
            nums = [weight_start - 1, weight_end + 1]
            while (max(nums) > weight_end) & (min(nums) < weight_start):
                nums = []
                while sum(nums) == 0:
    ...
    def get_efficient_frontier_and_CML(self):
        ...
        plt.ylim([np.quantile(self.portfolio_return, 0.005), np.quantile(self.portfolio_return, 0.995)])
        plt.show()
